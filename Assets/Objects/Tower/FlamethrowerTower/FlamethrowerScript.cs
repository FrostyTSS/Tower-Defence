...
public class FlamethrowerScript : BaseTower
{
    public bool DebugSearchEnemy = false;
    public float SpeedModifier = 0.45f;
    public float SpeedModifierLength = 2.5f;



    public override bool CheckEnemyTowerUnique(BaseEnemy enemytocheck) // use this for special tower checks ie if slowed, don't shoot if a flamethrower tower
    {
       // Debug.Log("Checking slow, and the result is " + enemytocheck.CurrentlySlowed);
        if (enemytocheck.CurrentlySlowed)
        {
            return true;
        }
        else
